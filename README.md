...
| [`open-pr`](open-pr/README.md) | Push the current branch and open a new pull request on GitHub using the `gh` CLI. Detects if a PR already exists and directs the user to use update-pr instead. |
| [`update-pr`](update-pr/README.md) | Regenerate and update an existing pull request's title and body on GitHub using the `gh` CLI. |
